...
TxController.prototype.transformTransaction = function (transaction, options, callback) {
    ...
    var txid = String(transformed.txid);
    var self = this;
    this.node.services.bitcoind.getsparknametxdetails(txid, function (err, result) {
        if (!err && result && result.name && result.address) {
            ...
            transformed.vout.forEach(function (vout, index) {
                vout.n = index;
            });
        }
        ...
        transformed.size = transaction.hex.length / 2;
        if (!transaction.coinbase) {
        ...
        if (transformed.extraPayload) {
            if(transaction.version >= 3 && transaction.type == 1) transformed.proReg = transaction.proReg;
            if(transaction.version >= 3 && transaction.type == 2) transformed.proUpServ = transaction.proUpServ;
            if(transaction.version >= 3 && transaction.type == 3) transformed.proUpReg = transaction.proUpReg;
            if(transaction.version >= 3 && transaction.type == 4) transformed.proUpRev = transaction.proUpRev;
            if(transaction.version >= 3 && transaction.type == 5) transformed.cbTx = transaction.cbTx;
            if(transaction.version >= 3 && transaction.type == 6) transformed.finalCommitment = transaction.finalCommitment;
            if(transaction.version >= 3 && transaction.type == 7) transformed.sporkTx = transaction.sporkTx;
        }
