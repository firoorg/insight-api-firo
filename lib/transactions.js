...
TxController.prototype.outputStatus = function (req, res) {
    var index = parseInt(req.params.index, 10);

    if (_.isNaN(index) || index < 0) {
        return res.status(400).send('Invalid output index');
    }

    if (!req.transaction || !req.transaction.vout || _.isUndefined(req.transaction.vout[index])) {
        return this.common.handleErrors(null, res);
    }

    var output = req.transaction.vout[index];
    var spent = !!output.spentTxId;
    var status = {
    ...
    if (spent) {
        status.spentTxId = output.spentTxId;
        status.spentIndex = output.spentIndex;
        status.spentHeight = output.spentHeight;
    }

    res.jsonp(status);
};

TxController.prototype.mempoolStatus = function (req, res) {
    if (!req.transaction || req.transaction.confirmations > 0) {
        return this.common.handleErrors(null, res);
    }
