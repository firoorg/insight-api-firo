...
AddressController.prototype.check = function (req, res, next, addresses) {
    ...
    for (var i = 0; i < addresses.length; i++) {
        try {
            ...
            if(addresses[i] === 'Remint'){
                continue;
            }
            if(addresses[i] === 'Lelantusmint' || addresses[i] === 'Lelantusjmint' || addresses[i] === 'Lelantusjsplit' || addresses[i] === 'Lelantus'){
                continue;
            }
            if(addresses[i] === 'Sparkmint' || addresses[i] === 'Sparksmint' || addresses[i] === 'Sparkspend' || addresses[i] === 'Spark'){
                continue;
            }
            if(addresses[i] === 'Sparkname' || addresses[i] === 'sparkname'){
                continue;
            }
          var a = new bitcore.Address(addresses[i]);
        } catch (e) {
            return self.common.handleErrors({
                message: 'Invalid address: ' + self.common.escapeHtml(e.message),
                code: 1
            }, res);
        }
    }
