...
class MongoConnector {
    ...
    invalidate() {
        ...
        let promise = this.blocks
            ...
            .then(block => {
                // delete all outputs belonged to latest block
                return this.outputs.deleteMany({ block: block.hash });
            });
    ...
    insert_block(block) {
        ...
        let promise = this.blocks
            ...
            .then(() => {
                // get outputs that spent by this block
                return this.getOutputs(inputs);
            })
            .then(spents => {
                // mark outputs that spent by this block
                return Promise.all(
                    spents.map(spent => {
                        return this.outputs.updateOne(
                            { tx: spent.tx, index: spent.index },
                            { $set: { spent: block.hash } }
                        );
                    })
                ).then(() => spents);
            })
            .then(spents => {
                return this.updateBalances(
                    this.calculateFinalBalance(outputs, spents)
                );
            });
