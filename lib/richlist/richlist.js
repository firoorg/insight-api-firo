...
class RichListController extends EventEmitter {
    /**
     * @param {object} options require node and conn is optional
     */
    constructor(options) {
        super();
    ...
    list(req, res) {
        this.conn.get_top(100).then(list => {
            if (list) {
                res.jsonp(list);
            }
        });
    ...
    startScanBlocks() {
        ...
        this.conn.bestBlock().then(local => {
            ...
            return new Promise((resolve, reject) => {
                this.node.getBlockHeader(local, (err, block) => {
                    if (err) {
            ...
            if (!block) {
                ...
                return this.conn.invalidate().then(() => {
            ...
            block.txids.forEach(txid => {
                promises.push(limie(() => this.getTransaction(txid)));
            });

            return Promise.all(promises).then(txs => ({ block, txs }));
        }).then(({ block, txs }) => {
            ...
            // previously there is a bug that allow one transaction to contained in more than one block
            // so we need this to handle that case
            block.txs = txs.filter(tx => tx.height === block.height);

            return this.conn.insert_block(block).then(() => block);
        }).then(block => {
